...
# /etc/bind
lxc -q exec ${CONTAINER_NAME} -- sh -c "tar -czf ${name_tar} /etc/apache2/conf-available/nagios4-cgi.conf /etc/apache2/mods-available/mime.conf /etc/nagios4/* /root/.config/* /root/.ssh/* /root/.bash_history /root/.bashrc /root/.profile > /dev/null"
if [[ $? -eq 0 ]]; then
